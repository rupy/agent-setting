...
[公開スキル39件](.)と[スキル以外の資料31件](non-skills/)の採否検討を、内容が重なるものは一項目にまとめた。判断は[現行スキル](../../../../skills/write-clear-japanese/SKILL.md)との照合に基づく。「既存で対応」は追加不要、「追加候補」は本文への追加を検討、「条件付き」は該当する文書や実例がある場合に扱うことを指す。元資料の効果を新たに実測した一覧ではない。
